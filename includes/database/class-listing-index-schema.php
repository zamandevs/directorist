...
class Listing_Index_Schema {
    ...
    public static function field_table() {
        ...
        return $wpdb->prefix . 'directorist_listing_field_exact_index';
    }

    public static function number_field_table() {
        global $wpdb;

        return $wpdb->prefix . 'directorist_listing_field_number_index';
    }

    public static function date_field_table() {
        global $wpdb;

        return $wpdb->prefix . 'directorist_listing_field_date_index';
    }

    public static function text_field_table() {
        global $wpdb;

        return $wpdb->prefix . 'directorist_listing_field_text_index';
    }

    public static function field_tables() {
        return [
            'exact'  => self::field_table(),
            'number' => self::number_field_table(),
            'date'   => self::date_field_table(),
            'text'   => self::text_field_table(),
        ];
    }
    ...
    public static function create() {
        ...
        $number_table    = self::number_field_table();
        $date_table      = self::date_field_table();
        $text_table      = self::text_field_table();
        $state_table     = self::state_table();

        require_once ABSPATH . 'wp-admin/includes/upgrade.php';

        if ( self::VERSION !== get_option( self::VERSION_OPTION, '' ) ) {
            self::drop_obsolete_field_tables();
        }
        ...
        $date_sql = "CREATE TABLE {$date_table} (
            listing_id bigint(20) unsigned NOT NULL,
            directory_id bigint(20) unsigned NOT NULL DEFAULT 0,
            generation bigint(20) unsigned NOT NULL DEFAULT 0,
            field_key varchar(128) NOT NULL,
        ...
        $text_sql = "CREATE TABLE {$text_table} (
            listing_id bigint(20) unsigned NOT NULL,
            directory_id bigint(20) unsigned NOT NULL DEFAULT 0,
            generation bigint(20) unsigned NOT NULL DEFAULT 0,
            field_key varchar(128) NOT NULL,
            field_type varchar(32) NOT NULL DEFAULT '',
            value_hash char(32) NOT NULL,
            search_document longtext NOT NULL,
            indexed_at datetime NOT NULL,
            PRIMARY KEY  (listing_id,generation,field_key,value_hash),
            KEY directory_generation_field (directory_id,generation,field_key(64),listing_id),
            FULLTEXT KEY directory_generation_field_search (search_document)
        ) {$charset_collate};";
    ...
    public static function tables_exist() {
        ...
        $tables   = array_merge( [ self::listing_table() ], array_values( self::field_tables() ), [ self::state_table() ] );
        $exist    = true;
    ...
    public static function verify_schema() {
        ...
        $required_columns = [
            ...
            self::field_table()        => [
                'listing_id', 'directory_id', 'generation', 'field_key', 'field_type', 'value_string', 'value_hash', 'indexed_at',
            ],
            self::number_field_table() => [
                'listing_id', 'directory_id', 'generation', 'field_key', 'field_type', 'value_hash', 'value_string', 'value_num', 'value_signed', 'indexed_at',
            ],
            self::date_field_table()   => [
                'listing_id', 'directory_id', 'generation', 'field_key', 'field_type', 'value_hash', 'value_string', 'value_date', 'indexed_at',
            ],
            self::text_field_table()   => [
                'listing_id', 'directory_id', 'generation', 'field_key', 'field_type', 'value_hash', 'search_document', 'indexed_at',
            ],
            self::state_table()        => [
    ...
    public static function is_ready() {
        ...
        if ( self::DATA_VERSION !== get_option( self::DATA_VERSION_OPTION, '' ) || self::STATUS_READY !== self::status() ) {
            return false;
        }
    ...
    private static function required_tables_exist() {
        ...
        if ( null === $count && $error ) {
            ...
            self::$tables_present[ $wpdb->prefix ] = count( $tables ) === (int) $count;
        }
    ...
    private static function required_indexes() {
        return [
            ...
            self::field_table()        => [
                'PRIMARY'                          => 'listing_id,generation,field_key,value_hash',
            ...
            self::text_field_table()   => [
                'PRIMARY'                           => 'listing_id,generation,field_key,value_hash',
                'directory_generation_field'        => 'directory_id,generation,field_key(64),listing_id',
                'directory_generation_field_search' => 'search_document',
            ],
            self::state_table()        => [
    ...
    private static function ensure_index_shapes() {
        ...
        $obsolete = [
            ...
            self::field_table()   => [
                'listing_field_value', 'directory_field_exact', 'listing_field', 'directory_field_string', 'directory_field_hash',
            ],
        ];
    ...
    private static function drop_obsolete_field_tables() {
        global $wpdb;

        $tables   = array_values( self::field_tables() );
        $tables[] = $wpdb->prefix . 'directorist_listing_field_index';

        foreach ( array_unique( $tables ) as $table ) {
            // All field tables are derived and remain disabled until a verified rebuild completes.
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.SchemaChange,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.PreparedSQL.NotPrepared
