...
class Listing_Index {
    ...
    public static function delete_listing( $post_id, $post ) {
        ...
        $wpdb->delete( Listing_Index_Schema::field_table(), [ 'listing_id' => $post_id ], [ '%d' ] );
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
    ...
    private static function verify_coverage() {
        ...
        $orphaned_fields = (int) $wpdb->get_var(
            $wpdb->prepare(
                'SELECT COUNT(f.id) FROM ' . Listing_Index_Schema::field_table() . " f LEFT JOIN {$wpdb->posts} p ON p.ID = f.listing_id AND p.post_type = %s WHERE p.ID IS NULL",
                ATBDP_POST_TYPE
            )
        );
    ...
    private static function sync_core_meta( $listing_id, $meta_key, $meta_value, $exists ) {
        ...
        if ( '_directory_type' === $meta_key ) {
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
            $wpdb->delete( Listing_Index_Schema::field_table(), [ 'listing_id' => $listing_id ], [ '%d' ] );
    ...
    private static function build_field_row( $listing_id, $directory_id, $generation, array $definition, $value ) {
        ...
        if ( ! $is_text && strlen( $normalized ) > 191 ) {
            return null;
        }

        $search_document = $is_text ? Listing_Index_Text::document( $normalized ) : null;
        ...
        $row = [
            ...
            'search_scope'    => $is_text ? Listing_Index_Text::scope_token( $directory_id, $generation, $definition['field_key'] ) : '',
    ...
    private static function insert_field_rows( array $rows ) {
        ...
        foreach ( array_chunk( $rows, 200 ) as $chunk ) {
            ...
            foreach ( $chunk as $row ) {
                $num_placeholder    = null === $row['value_num'] ? 'NULL' : '%.10f';
                $signed_placeholder = null === $row['value_signed'] ? 'NULL' : '%d';
                $date_placeholder   = null === $row['value_date'] ? 'NULL' : '%s';
                $search_placeholder = null === $row['search_document'] ? 'NULL' : '%s';
                $placeholders[]     = "(%d,%d,%d,%s,%s,%s,%s,%s,{$search_placeholder},{$num_placeholder},{$signed_placeholder},{$date_placeholder},%s)";
                $values[]           = $row['listing_id'];
                $values[]           = $row['directory_id'];
                $values[]           = $row['generation'];
                $values[]           = $row['field_key'];
                $values[]           = $row['field_type'];
                $values[]           = $row['value_string'];
                $values[]           = $row['value_hash'];
                $values[]           = $row['search_scope'];

                if ( null !== $row['search_document'] ) {
                    $values[] = $row['search_document'];
                }

                if ( null !== $row['value_num'] ) {
                    $values[] = $row['value_num'];
                }

                if ( null !== $row['value_signed'] ) {
            ...
            $sql  = 'INSERT INTO ' . Listing_Index_Schema::field_table();
            $sql .= ' (listing_id,directory_id,generation,field_key,field_type,value_string,value_hash,search_scope,search_document,value_num,value_signed,value_date,indexed_at) VALUES ';
    ...
    private static function field_rows_match( $listing_id, $directory_id, $generation, array $definitions = [] ) {
        ...
        $expected = array_map(
            static function( $row ) {
                return $row['field_key'] . ':' . $row['value_hash'];
            },
            self::build_all_field_rows( $listing_id, $directory_id, $generation, $definitions )
        );
        ...
        $stored = $wpdb->get_results(
            $wpdb->prepare(
                'SELECT field_key, value_hash FROM ' . Listing_Index_Schema::field_table() . ' WHERE listing_id = %d AND generation = %d',
                $listing_id,
                $generation
            ),
            ARRAY_A
