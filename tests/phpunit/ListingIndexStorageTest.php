...
use Directorist\database\Listing_Index_Text;
...
class Directorist_Listing_Index_Storage_Test extends WP_UnitTestCase {
    ...
    public function test_schema_is_idempotent_and_contains_query_indexes() {
        ...
        $number_indexes = $wpdb->get_col( 'SHOW INDEX FROM ' . Listing_Index_Schema::number_field_table(), 2 );
        $date_indexes   = $wpdb->get_col( 'SHOW INDEX FROM ' . Listing_Index_Schema::date_field_table(), 2 );
        $text_indexes   = $wpdb->get_col( 'SHOW INDEX FROM ' . Listing_Index_Schema::text_field_table(), 2 );
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
        $field_index_rows = $wpdb->get_results( 'SHOW INDEX FROM ' . Listing_Index_Schema::field_table(), ARRAY_A );
        $text_index_rows  = $wpdb->get_results( 'SHOW INDEX FROM ' . Listing_Index_Schema::text_field_table(), ARRAY_A );
        ...
        $this->assertNotContains( 'directory_generation_field_signed', $field_indexes );
        $this->assertNotContains( 'directory_generation_field_search', $field_indexes );
        $this->assertContains( 'directory_generation_field_num', $number_indexes );
        $this->assertContains( 'directory_generation_field_signed', $number_indexes );
        $this->assertContains( 'directory_generation_field_date', $date_indexes );
        $this->assertContains( 'directory_generation_field_search', $text_indexes );
        $this->assertNotContains( 'directory_generation_field_date', $number_indexes );
        $this->assertNotContains( 'directory_generation_field_num', $date_indexes );
        $this->assertNotContains( 'directory_generation_field_exact', $text_indexes );
        ...
        $this->assertSame( [ 'search_document' ], wp_list_pluck( $search_index, 'Column_name' ) );
        ...
        foreach ( Listing_Index_Schema::field_tables() as $field_table ) {
            $primary_rows = array_values(
                array_filter(
                    $wpdb->get_results( 'SHOW INDEX FROM ' . $field_table, ARRAY_A ),
                    static function( $index ) {
                        return 'PRIMARY' === $index['Key_name'];
                    }
                )
            );

            $this->assertSame( [ 'listing_id', 'generation', 'field_key', 'value_hash' ], wp_list_pluck( $primary_rows, 'Column_name' ) );
        }
    }

    public function test_fulltext_tokens_are_isolated_by_directory_generation_and_field() {
        $document = Listing_Index_Text::document( 10, 2, '_custom-text', 'Quiet river view' );

        $this->assertSame( '+' . str_replace( ' ', ' +', $document ), Listing_Index_Text::boolean_query( 10, 2, '_custom-text', 'Quiet river view' ) );
        $this->assertNotSame( $document, Listing_Index_Text::document( 11, 2, '_custom-text', 'Quiet river view' ) );
        $this->assertNotSame( $document, Listing_Index_Text::document( 10, 3, '_custom-text', 'Quiet river view' ) );
        $this->assertNotSame( $document, Listing_Index_Text::document( 10, 2, '_custom-url', 'Quiet river view' ) );
    }
    ...
    public function test_typed_field_schema_upgrade_requires_derived_data_rebuild() {
        global $wpdb;

        $listing_id = $this->create_listing( 'Schema Upgrade Listing' );
        Listing_Index::sync_listing( $listing_id );

        // Seed derived field data directly so this test only exercises the
        // schema-upgrade contract, independent of directory field fixtures.
        $wpdb->insert(
            ...
            [
                ...
                'directory_id' => 0,
                'generation'   => 0,
                'field_key'    => '_upgrade-test',
                'field_type'   => 'select',
                'value_string' => 'legacy',
                'value_hash'   => md5( 'legacy' ),
                'indexed_at'   => current_time( 'mysql', true ),
            ],
            [ '%d', '%d', '%d', '%s', '%s', '%s', '%s', '%s' ]
        );

        $this->assertGreaterThan( 0, $this->count_all_field_rows() );

        update_option( Listing_Index_Schema::VERSION_OPTION, '2.1.0', false );
        update_option( Listing_Index_Schema::DATA_VERSION_OPTION, '2.1.0', false );
        Listing_Index_Schema::mark_status( Listing_Index_Schema::STATUS_READY );
        Listing_Index_Schema::reset_request_cache();

        $dropped_tables = [];
        $capture_query  = static function( $query ) use ( &$dropped_tables ) {
            if ( 0 === strpos( $query, 'DROP TEMPORARY TABLE IF EXISTS ' ) ) {
                $dropped_tables[] = substr( $query, strlen( 'DROP TEMPORARY TABLE IF EXISTS ' ) );
            }
        ...
        $this->assertSame( '2.1.0', get_option( Listing_Index_Schema::DATA_VERSION_OPTION ) );
        ...
        foreach ( Listing_Index_Schema::field_tables() as $field_table ) {
            $this->assertContains( $field_table, $dropped_tables );
        }
    }
    ...
    public function test_custom_field_sync_uses_search_builder_definitions() {
        ...
        $exact_rows  = $wpdb->get_results(
            $wpdb->prepare(
                'SELECT field_key, field_type, value_string FROM ' . Listing_Index_Schema::field_table() . ' WHERE listing_id = %d ORDER BY field_key, value_string',
                $listing_id
            ),
            ARRAY_A
        );
        $number_rows = $wpdb->get_results(
            $wpdb->prepare(
                'SELECT field_key, field_type, value_string, value_num FROM ' . Listing_Index_Schema::number_field_table() . ' WHERE listing_id = %d ORDER BY field_key',
                $listing_id
            ),
            ARRAY_A
        );
        $text_rows   = $wpdb->get_results(
            $wpdb->prepare(
                'SELECT field_key, field_type, search_document FROM ' . Listing_Index_Schema::text_field_table() . ' WHERE listing_id = %d ORDER BY field_key',
                $listing_id
            ),
            ARRAY_A
        );
        ...
        $all_field_keys = array_merge( wp_list_pluck( $exact_rows, 'field_key' ), wp_list_pluck( $number_rows, 'field_key' ), wp_list_pluck( $text_rows, 'field_key' ) );

        $this->assertCount( 3, $exact_rows );
        $this->assertCount( 1, $number_rows );
        ...
        $this->assertNotContains( '_display-only', $all_field_keys );

        foreach ( [ '_custom-text-1', '_custom-textarea-1', '_custom-url-1' ] as $field_key ) {
            $matching_text_rows = array_values(
                array_filter(
                    $text_rows,
            ...
            $this->assertCount( 1, $matching_text_rows );
            $this->assertNotEmpty( $matching_text_rows[0]['search_document'] );
            $this->assertMatchesRegularExpression( '/^dt[a-f0-9]{32}(?: dt[a-f0-9]{32})*$/', $matching_text_rows[0]['search_document'] );
        }
        ...
        $original_text_document = $wpdb->get_var(
            $wpdb->prepare(
                'SELECT search_document FROM ' . Listing_Index_Schema::text_field_table() . ' WHERE listing_id = %d AND field_key = %s',
                $listing_id,
                '_custom-text-1'
            )
        );
        ...
        $this->assertSame(
            ...
            $wpdb->get_var(
                $wpdb->prepare(
                    'SELECT COUNT(*) FROM ' . Listing_Index_Schema::text_field_table() . ' WHERE listing_id = %d AND field_key = %s',
                    $listing_id,
                    '_custom-text-1'
                )
            )
    ...
    public function test_multisite_tables_and_schema_health_use_current_blog_prefix() {
        ...
        $secondary_tables = array_merge(
            [ Listing_Index_Schema::listing_table() ],
            array_values( Listing_Index_Schema::field_tables() ),
    ...
    private function count_all_field_rows() {
        return array_sum( $this->field_row_counts() );
    }

    private function field_row_counts() {
        global $wpdb;

        $counts = [];

        foreach ( Listing_Index_Schema::field_tables() as $type => $field_table ) {
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQL.NotPrepared
            $counts[ $type ] = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$field_table}" );
        }
