...
class Directorist_Listing_Index_Maintenance_Test extends WP_UnitTestCase {
    ...
    public function test_queued_directory_rebuild_switches_generation_after_verification() {
        ...
        $rows = $wpdb->get_results(
            $wpdb->prepare(
                '(SELECT field_key, generation FROM ' . Listing_Index_Schema::field_table() . ' WHERE listing_id = %d) UNION ALL (SELECT field_key, generation FROM ' . Listing_Index_Schema::number_field_table() . ' WHERE listing_id = %d) ORDER BY field_key',
                $listing_id,
                $listing_id
            ),
            ARRAY_A
    ...
    public function test_listing_mutations_update_active_and_pending_generations() {
        ...
        $rows  = $wpdb->get_results(
            $wpdb->prepare(
                '(SELECT generation, field_key, value_string FROM ' . Listing_Index_Schema::field_table() . ' WHERE listing_id = %d) UNION ALL (SELECT generation, field_key, value_string FROM ' . Listing_Index_Schema::number_field_table() . ' WHERE listing_id = %d) ORDER BY generation, field_key',
                $listing_id,
                $listing_id
            ),
            ARRAY_A
    ...
    public function test_new_configuration_supersedes_and_cleans_an_older_pending_generation() {
        ...
        $old_pending_rows = $this->count_directory_field_rows( $directory_id, $first_pending['pending_generation'] );
    ...
    public function test_deleting_a_directory_removes_only_its_derived_state_and_fields() {
        ...
        $deleted_fields = $this->count_directory_field_rows( $directory_id );
    ...
    private function count_directory_field_rows( $directory_id, $generation = null ) {
        global $wpdb;

        $count = 0;

        foreach ( Listing_Index_Schema::field_tables() as $field_table ) {
            $sql    = "SELECT COUNT(*) FROM {$field_table} WHERE directory_id = %d";
            $values = [ $directory_id ];

            if ( null !== $generation ) {
                $sql     .= ' AND generation = %d';
                $values[] = $generation;
            }
