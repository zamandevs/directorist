...
class Directorist_Single_Listing_Behavior_Test extends WP_UnitTestCase {
    ...
    public function tear_down() {
        if ( $this->term_metadata_filter ) {
            remove_filter( 'get_term_metadata', $this->term_metadata_filter, 10 );
            $this->term_metadata_filter = null;
        }
    ...
    public function test_falsey_options_and_directory_options_are_cached_until_reset() {
        $options                         = get_option( 'atbdp_option', [] );
        $options['phase4_falsey_option'] = 0;
        update_option( 'atbdp_option', $options );
        update_term_meta( $this->directory_id, 'phase4_type_option', 'first' );
        ...
        add_filter(
            'directorist_option',
            static function ( $value, $name ) use ( &$option_calls ) {
                if ( 'phase4_falsey_option' === $name ) {
                    ++$option_calls;
                }
        ...
        $this->term_metadata_filter = static function ( $value, $object_id, $meta_key ) use ( &$type_calls ) {
            if ( 'phase4_type_option' === $meta_key ) {
                ++$type_calls;
            }

            return $value;
        };
        add_filter( 'get_term_metadata', $this->term_metadata_filter, 10, 3 );

        $listing = Directorist_Single_Listing::instance( $this->listing_id );

        $this->assertSame( 0, $listing->get_hydrated_option( 'phase4_falsey_option', 1 ) );
        $this->assertSame( 0, $listing->get_hydrated_option( 'phase4_falsey_option', 1 ) );
        $this->assertSame( 'first', $listing->get_hydrated_type_option( 'phase4_type_option' ) );
        $this->assertSame( 'first', $listing->get_hydrated_type_option( 'phase4_type_option' ) );
        $this->assertSame( 1, $option_calls );
        $this->assertSame( 1, $type_calls );

        $options['phase4_falsey_option'] = 1;
        update_option( 'atbdp_option', $options );
        update_term_meta( $this->directory_id, 'phase4_type_option', 'second' );
        $type_calls_before_reset = $type_calls;
        ...
        $this->assertSame( 1, $listing->get_hydrated_option( 'phase4_falsey_option', 0 ) );
        $this->assertSame( 'second', $listing->get_hydrated_type_option( 'phase4_type_option' ) );
        $this->assertSame( 2, $option_calls );
        $this->assertSame( $type_calls_before_reset + 1, $type_calls );
    }
    ...
    public function test_missing_custom_single_page_returns_empty_content() {
        update_term_meta( $this->directory_id, 'enable_single_listing_page', true );
        update_term_meta( $this->directory_id, 'single_listing_page', PHP_INT_MAX );

        $listing = Directorist_Single_Listing::instance( $this->listing_id );

        $this->assertSame( '', $listing->single_page_content() );
        $this->assertSame( '', $listing->single_page_content() );
    }
    ...
    public function test_map_section_visibility_and_field_rendering_keep_distinct_rules() {
        update_post_meta( $this->listing_id, '_address', 'Address without coordinates' );
        $this->reset_single_listing();
        ...
        $this->assertTrue( $listing->get_field_state( $field )['section_has_value'] );
        $this->assertFalse( $listing->get_field_state( $field )['value'] );

        update_post_meta( $this->listing_id, '_manual_lat', '23.8103' );
        update_post_meta( $this->listing_id, '_manual_lng', '90.4125' );
        update_post_meta( $this->listing_id, '_hide_map', 1 );
        $listing->reset_hydrated_data();

        $this->assertTrue( $listing->get_field_state( $field )['section_has_value'] );
        $this->assertFalse( $listing->get_field_state( $field )['value'] );

        update_post_meta( $this->listing_id, '_hide_map', 0 );
        $listing->reset_hydrated_data();

        $this->assertTrue( $listing->get_field_state( $field )['value'] );
    }
    ...
    public function test_slider_data_cache_separates_distinct_argument_sets() {
        $listing            = Directorist_Single_Listing::instance( $this->listing_id );
        $without_thumbnails = $listing->get_slider_data( [ 'footer_thumbnail' => 0 ] );
        $with_thumbnails    = $listing->get_slider_data( [ 'footer_thumbnail' => 1 ] );

        $this->assertSame( '0', $without_thumbnails['show-thumbnails'] );
        $this->assertSame( '1', $with_thumbnails['show-thumbnails'] );
        $this->assertSame( $without_thumbnails, $listing->get_slider_data( [ 'footer_thumbnail' => 0 ] ) );
        $this->assertSame( $with_thumbnails, $listing->get_slider_data( [ 'footer_thumbnail' => 1 ] ) );
    }
    ...
    public function test_related_listing_filter_can_restore_found_rows() {
        add_filter(
            'directorist_related_listing_args',
        ...
        $this->assertFalse( $related->query_args['no_found_rows'] );
    }
