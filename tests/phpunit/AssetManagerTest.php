...
class Directorist_Asset_Manager_Test extends WP_UnitTestCase {
    ...
    public function test_manual_legacy_template_hooks_preserve_map_archive_and_search_assets() {
    ...
    public function test_standard_template_hook_pair_processes_asset_requirements_once() {
        $records  = [];
        $listener = static function ( $record ) use ( &$records ) {
            if ( 'map' === $record['asset'] && 'asset' === $record['type'] ) {
                $records[] = $record;
            }
        };
        $template = 'archive/map-view';
        $file     = ATBDP_DIR . 'templates/archive/map-view.php';
        $context  = [
            'source'   => 'core',
            'template' => $template,
            'file'     => $file,
        ...
        do_action( 'directorist_before_template_render', $template, $file, [], $context );
        \Directorist\Asset_Loader\Asset_Loader::suppress_next_legacy_template_hook( $template, $file );
        do_action( 'before_directorist_template_loaded', $template, $file, [] );
    ...
    public function test_legacy_hook_suppression_uses_counters_for_repeated_templates() {
        $records  = [];
        $listener = static function ( $record ) use ( &$records ) {
            if ( 'map' === $record['asset'] && 'asset' === $record['type'] ) {
                $records[] = $record;
    ...
    public function test_normalized_hook_reentrancy_cannot_consume_core_legacy_suppression() {
        $records                = [];
        $template               = 'archive/map-view';
        $file                   = ATBDP_DIR . 'templates/archive/map-view.php';
        $record_listener        = static function ( $record ) use ( &$records ) {
            if ( 'map' === $record['asset'] && 'asset' === $record['type'] ) {
                $records[] = $record;
            }
        };
        $nested_legacy_listener = static function ( $hook_template, $hook_file ) {
            do_action( 'before_directorist_template_loaded', $hook_template, $hook_file, [] );
        };

        add_action( 'directorist_asset_requirement_recorded', $record_listener );
        add_action( 'directorist_before_template_render', $nested_legacy_listener, 20, 2 );

        do_action(
            'directorist_before_template_render',
            $template,
            $file,
            [],
            [
        ...
        remove_action( 'directorist_before_template_render', $nested_legacy_listener, 20 );
        remove_action( 'directorist_asset_requirement_recorded', $record_listener );

        $this->assertCount( 2, $records );
    }

    public function test_legacy_template_hook_still_reaches_third_party_callbacks_once() {
        $calls    = 0;
        $listener = static function () use ( &$calls ) {
            $calls++;
        };

        add_action( 'before_directorist_template_loaded', $listener, 20 );

        do_action( 'before_directorist_template_loaded', 'archive/map-view' );

        remove_action( 'before_directorist_template_loaded', $listener, 20 );
