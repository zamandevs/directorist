...
class Directorist_Request_Registration_Test extends WP_UnitTestCase {
    ...
    public function test_wordpress_lifecycle_hooks_are_registered_outside_admin_screens() {
        set_current_screen( 'front' );

        $custom_post     = directorist()->custom_post;
        $custom_taxonomy = directorist()->taxonomy;
        $user            = directorist()->user;

        $this->assertSame( 10, has_filter( 'manage_' . ATBDP_POST_TYPE . '_posts_columns', [ $custom_post, 'add_new_listing_columns' ] ) );
        $this->assertSame( 10, has_action( 'save_post', [ ATBDP_Custom_Post::class, 'on_save_post' ] ) );
        $this->assertSame( 10, has_filter( 'manage_edit-' . ATBDP_CATEGORY . '_columns', [ $custom_taxonomy, 'register_category_columns' ] ) );
        $this->assertSame( 10, has_filter( 'term_updated_messages', [ $custom_taxonomy, 'add_term_updated_messages' ] ) );
        $this->assertSame( 10, has_filter( 'pre_get_posts', [ $user, 'restrict_listing_to_the_author' ] ) );
        $this->assertSame( 10, has_action( 'admin_footer', [ Directorist\Widgets\Widget_Fields::class, 'load_scripts' ] ) );
    }
}
